...
bool Seed::clustering(ofstream& temOut)
{
	...
	for(unordered_map<int,int>::const_iterator iterS = this ->seed_list.begin(); iterS != this ->seed_list.end(); iterS++)
    {
        ...
        if((this->relate_hash_p)->count(iterS->first) > 0 )
        {
            ...
            {
                if(this->seed_list.find(*iter3) == this->seed_list.end())
                {
	...
	if(pre_assistant_list.size() > 1500)
	{
		cerr << "opt_list too big before MCL, try to pick 1500" << endl;
		//pick 4000 barcode in pre_assistant_list
		unordered_map<int, int> tmpMap;
		for(int i = 0; i <1500; i++)
		{
			tmpMap.insert(make_pair(pre_assistant_list.begin()->first, pre_assistant_list.begin()->second));
			pre_assistant_list.erase(pre_assistant_list.begin());
		}
		pre_assistant_list.clear();
		pre_assistant_list = tmpMap;
	}
