...
struct UniqueKmerArray
{
    ...
    {
        if ( ! ( index < num_kmer &&  *(bytes + index * (max_kmer_count +1))< max_kmer_count) )
	{
		fprintf(stderr," index %lu , number_kmer %lu\n",index, num_kmer);
		fprintf(stderr," count %d ,  max %d\n" , *(bytes + index * (max_kmer_count +1)),max_kmer_count );
		assert(0);
	}
